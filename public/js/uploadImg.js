$().ready(function(){
    var imgFun={
        showFun:function(e,imgName){
            var val = e.value;
            var upLoadType = '.jpg,.png';//['.jpg','.gif','.bmp','.png']; //可上传的格式
            ...
            if($('#shang').val().length&&$('#xia').val().length&&$('#zuo').val().length&&$('#you').val().length&&$('#qian').val().length&&$('#hou').val().length){
                ...
                success: function(data) {  
                    if(data.code==1){
                        ...
                        queryMsg.push({'name':'shang','value':data.msg[0]});
                        queryMsg.push({'name':'xia','value':data.msg[1]});
                        queryMsg.push({'name':'zuo','value':data.msg[2]});
                        queryMsg.push({'name':'you','value':data.msg[3]});
                        queryMsg.push({'name':'qian','value':data.msg[4]});
                        queryMsg.push({'name':'hou','value':data.msg[5]});
                        $.ajax({  
